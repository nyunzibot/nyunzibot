...
def setup(bot: discord.Client):
    ...
    async def kiss(interaction: discord.Interaction, target: discord.User, extra_tags: Optional[str] = None):
        ok = await safe_defer(interaction, thinking=True)
        if not ok:
            return

        log.info("[CMD] /kiss actor=%s target=%s", interaction.user.id, target.id)

        if target.id == interaction.user.id:
            await interaction.followup.send("No self-kisses... try kissing someone else! 😘", ephemeral=True)
            return

        view = KissView(interaction.user, target, extra_tags=extra_tags or "")
        ...
        view.seen.add(md5)
        await STATS_DB.record_action("kiss", interaction.user.id, target.id, is_back=False)
        count = await STATS_DB.get_pair_count("kiss", interaction.user.id, target.id)
