...
from views.tuck_view import TuckView
...
def setup(bot: discord.Client):
    @bot.tree.command(name="tuck", description="[SFW] Tuck another user into bed with cozy anime images")
    ...
    async def tuck(interaction: discord.Interaction, target: discord.User, extra_tags: Optional[str] = None):
        ok = await safe_defer(interaction, thinking=True)
        if not ok:
            return

        log.info("[CMD] /tuck actor=%s target=%s", interaction.user.id, target.id)

        if target.id == interaction.user.id:
            await interaction.followup.send("You tuck yourself into bed! 🛏️ Sweet dreams~", ephemeral=True)
            return

        view = TuckView(interaction.user, target, extra_tags=extra_tags or "")
        ...
        tags = build_tag_ladder(TUCK_BASE, TUCK_POSITIVE_SETS, negative_tags=NEGATIVE_TAGS_SFW)
        tags = _apply_extra_to_ladder(tags, extra_tags or "")
        result = await pick_media_sfw(tags, view.seen, tries=8, status_cb=on_status, category="tuck")
        ...
        view.seen.add(md5)
        await STATS_DB.record_action("tuck", interaction.user.id, target.id, is_back=False)
        count = await STATS_DB.get_pair_count("tuck", interaction.user.id, target.id)
        totals = await STATS_DB.get_user("tuck", target.id)
        target_total = int(totals.get("received", 0))

        line = random.choice(TUCK_LINES).format(actor=f"**{interaction.user.display_name}**", target=f"**{target.display_name}**")
