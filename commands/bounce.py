...
from tags.tag_sets import BOUNCE_BASE, BOUNCE_POSITIVE_SETS, NEGATIVE_TAGS, ALLOWED_OVERRIDES
...
def setup(bot: discord.Client):
    ...
    async def bounce(interaction: discord.Interaction, target: discord.User, extra_tags: Optional[str] = None):
        ...
        log.info("[CMD] /bounce actor=%s target=%s", interaction.user.id, target.id)

        if target.id == interaction.user.id:
            await interaction.followup.send("You can't bounce on yourself 😅", ephemeral=True)
            return
        ...
        #await send_dm_notify("bounce", interaction.user, target)
