...
from tags.tag_sets import TUCK_BASE, TUCK_POSITIVE_SETS, NEGATIVE_TAGS_SFW
...
from text.tuck_lines import TUCK_LINES
from ui.embeds import build_action_embed

log = logging.getLogger("nyunzi")


class TuckView(discord.ui.View):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if child.label and "Refresh" in child.label:
                    child.custom_id = f"act:tuck:reroll:{self.original_actor.id}:{self.original_target.id}:{safe_tags}"
                elif child.label and "back" in child.label.lower():
                    child.custom_id = f"act:tuck:back:{self.original_actor.id}:{self.original_target.id}:{safe_tags}"
    ...
    async def reroll(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        tags = self._apply_extra_to_ladder(build_tag_ladder(TUCK_BASE, TUCK_POSITIVE_SETS, negative_tags=NEGATIVE_TAGS_SFW))
        picked, fetch_error = await pick_image_sfw(tags, self.seen, category="tuck")
        if not picked:
        ...
        line = random.choice(TUCK_LINES).format(
            actor=f"**{self.original_actor.display_name}**",
            target=f"**{self.original_target.display_name}**"
        )
        count = await STATS_DB.get_pair_count("tuck", self.original_actor.id, self.original_target.id)
        totals = await STATS_DB.get_user("tuck", self.original_target.id)
        ...
        try:
            ...
            if e.code == 40005:  # Payload Too Large
                log.warning("[TUCK VIEW] File too large for Discord")
    ...
    @discord.ui.button(label="Tuck back", emoji="🛏️", style=discord.ButtonStyle.primary)
    async def tuck_back(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.seen.add(md5)
        await STATS_DB.record_action("tuck", interaction.user.id, self.original_actor.id, is_back=True)
        count = await STATS_DB.get_pair_count("tuck", interaction.user.id, self.original_actor.id)
        totals = await STATS_DB.get_user("tuck", self.original_actor.id)
        target_total = int(totals.get("received", 0))
        ...
        new_view = TuckView(interaction.user, self.original_actor, extra_tags=self.extra_tags)
        new_view.seen = self.seen

        for item in new_view.children:
            if isinstance(item, discord.ui.Button) and item.label == "Tuck back":
                item.label = "Tuck again"
