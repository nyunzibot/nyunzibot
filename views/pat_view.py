...
class PatView(discord.ui.View):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if child.label and "Refresh" in child.label:
                    child.custom_id = f"act:pat:reroll:{self.original_actor.id}:{self.original_target.id}:{safe_tags}"
                elif child.label and "back" in child.label.lower():
                    child.custom_id = f"act:pat:back:{self.original_actor.id}:{self.original_target.id}:{safe_tags}"
    ...
    async def reroll(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        picked, fetch_error = await pick_image_sfw(tags, self.seen, category="pat")
        if not picked:
