        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if child.label and "Refresh" in child.label:
                    child.custom_id = f"act:cuddle:reroll:{self.original_actor.id}:{self.original_target.id}:{safe_tags}"
                elif child.label and "back" in child.label.lower():
                    child.custom_id = f"act:cuddle:back:{self.original_actor.id}:{self.original_target.id}:{safe_tags}"
