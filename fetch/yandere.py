...
# =========================
# YANDERE FETCH (XML -> JSON)
# Strategy: Probe Count (XML) -> Random PID (JSON)
# =========================
async def fetch_image_yandere(tags: str, avoid_md5s: set[str]) -> tuple[str, str | None, str] | None:
                    ...
                    try:
                        async with session.get(
                            ...
                            if resp.status == 429:
                                ...
                                continue
                            if resp.status != 200:
                                log.info("[YANDE PROBE] tier=%s status=%s (Retrying)", tier_label, resp.status)
                                continue
                            ...
                            if c:
                                ...
                                log.info("[YANDE PROBE] tier=%s status=%s count=%s", tier_label, resp.status, c)
                            else:
                                log.info("[YANDE PROBE] tier=%s status=%s count=None", tier_label, resp.status)
                            break
                    except Exception as e:
                        log.warning("[YANDE PROBE] probe error: %s", e)
                        await asyncio.sleep(backoffs[min(attempt, len(backoffs) - 1)])
                        continue
            else:
                if count:
            ...
            for attempt in range(1, MAX_ATTEMPTS + 1):
                ...
                try:
                    async with session.get(
                        ...
                        return (url, md5, "yandere")
                except Exception as e:
                    log.warning("[YANDE FETCH] fetch error: %s", e)
                    continue
